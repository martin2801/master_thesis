...
def main():
    ...
    ax.set_ylabel("SNPs per window", fontsize=14)
    ax.set_title(f"Before recombination removal  (raw snippy alignment, window = {args.window_size} bp)", fontsize=16)
    ...
    ax.set_ylabel("SNPs per window", fontsize=14)
    ax.set_title(f"After recombination removal  (masked alignment, window = {args.window_size} bp)", fontsize=16)
    ax.set_xlim(0, aln_len)
    ax.tick_params(labelsize=12)

    # Panel 3: recombinant region track (Gubbins only)
    ax = axes[2]
    for start, end in gubbins_regions:
        ax.barh(0.5, end - start, left=start, height=0.4, color="red", alpha=0.7)
    ax.set_yticks([])
    ax.set_xlabel("Alignment position (bp)", fontsize=14)
    ax.set_title("Recombinant regions", fontsize=16)
    ax.set_xlim(0, aln_len)
    ax.tick_params(labelsize=12)
    ...
    axes[2].legend(handles=patches, loc="upper right", fontsize=12)

    plt.tight_layout()
    plt.savefig(args.output, dpi=args.dpi, bbox_inches="tight", transparent=True)
