...
fig, ax = plt.subplots(figsize=(fig_width, fig_height), facecolor="#0F1117")
...
def draw_clade(clade, parent_x=None):
    ...
    lw = 0.4 if color == DEFAULT_COLOR else 1.0
    alpha = 0.5 if color == DEFAULT_COLOR else 0.95
...
ax.set_title("Phylogenetic tree — selective sweeps highlighted",
             color="white", fontsize=11, fontfamily="monospace", pad=12)
...
bar_y = -1
# line with end ticks
ax.plot([bar_x, bar_x + bar_len], [bar_y, bar_y], color="#aaaaaa", lw=1.5, solid_capstyle="butt")
ax.plot([bar_x, bar_x], [bar_y - 0.3, bar_y + 0.3], color="#aaaaaa", lw=1.5)
ax.plot([bar_x + bar_len, bar_x + bar_len], [bar_y - 0.3, bar_y + 0.3], color="#aaaaaa", lw=1.5)
