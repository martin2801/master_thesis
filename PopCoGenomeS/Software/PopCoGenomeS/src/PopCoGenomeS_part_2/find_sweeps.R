...
lbfile=read.table(paste0(basename,'.length_bias_500.txt'),sep='\t',header = T, stringsAsFactors = FALSE)
...
lbfile_1=lbfile%>% 
  ...
  mutate(Strain.1=as.character(Strain.1), Strain.2=as.character(Strain.2))
      ...
      if (file.size(tree_file) != 0L){
        ...
        for (n in 2:t$Nnode){ #because 1 is always root, and we are only looking at nodes with sister nodes
          ...
          if (sister_n>ntips){
            ...
            all_diff_clade_tips=expand.grid(tips_sister_st,tips_st, stringsAsFactors=FALSE)
