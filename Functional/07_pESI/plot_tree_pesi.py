...
parser.add_argument("--pesi", default=None,
                    help="Optional file listing genomes with pESI present, one genome name "
                         "per line (blank lines and lines starting with # are ignored). "
                         "Marked with an open ring around the tip, independent of sweep color.")
...
# --- Load pESI presence/absence (independent of sweep membership) ---
# A genome can be both in a sweep AND pESI+, so this is kept as a separate
# overlay marker rather than folded into the sweep color logic.
pesi_genomes = set()
if args.pesi:
    with open(args.pesi) as f:
        for line in f:
            g = line.strip()
            if g and not g.startswith("#"):
                pesi_genomes.add(g)
    print(f"Loaded {len(pesi_genomes)} pESI+ genomes from {args.pesi}")
...
PESI_COLOR    = "#D62839"   # open-ring overlay marking pESI+ genomes
...
def draw_clade(clade, parent_x=None):
    ...
    if not clade.is_terminal():
        ...
        # pESI overlay ring - drawn regardless of sweep/sister status so it
        # never competes with or hides the sweep color underneath it
        if clade.name in pesi_genomes:
            ax.scatter(x, y, s=34, facecolors="none", edgecolors=PESI_COLOR,
                       marker="o", linewidths=0.8, zorder=8)
...
if args.pesi:
    legend_patches.append(
        Line2D([0], [0], marker="o", linestyle="none", markersize=6.5,
               markerfacecolor="none", markeredgecolor=PESI_COLOR,
               markeredgewidth=1.2, label=f"pESI+ (n={len(pesi_genomes)})")
    )
